...
export function Options() {
  ...
  const [hideZeroLots, setHideZeroLots] = useState(false);
  ...
  const fetchManualStrikePrice = async (symbol: string) => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        return {
          manualStrikePrice: data.manualAverageStrikePrice ? parseFloat(String(data.manualAverageStrikePrice)) : null,
          manualOptionContracts: data.manualOptionContracts ? parseInt(String(data.manualOptionContracts)) : null
        };
      }
    } catch (error) {
  ...
  const getMonthNames = () => {
    ...
    const lastMonth = new Date(now.getFullYear(), now.getMonth() - 1, 1).toLocaleString('default', { month: 'long' });
    const twoMonthsAgo = new Date(now.getFullYear(), now.getMonth() - 2, 1).toLocaleString('default', { month: 'long' });
    ...
    symbolsWithOptions.forEach(symbol => {
      ...
      symbolTransactions.forEach(transaction => {
        ...
        if (transaction.security_type === 'OPTN') {
          ...
          if (transDate >= dateRanges.weekAgo) {
      ...
      analysis.currentBasisSharePrice = lastEquityTransaction ? parseFloat(String(lastEquityTransaction.price)) : 0;
      analysis.currentBasisSharePrice = (analysis.currentExposure - analysis.netPremiumAllTime) / analysis.totalLots / 100 || 0;
      ...
      analysis.averageStrikePrice = analysis.manualStrikePrice || (strikeArray.length > 0 
        ? strikeArray.reduce((sum, strike) => sum + strike, 0) / strikeArray.length 
        : 0);

      // Calculate absolute premium gain percentage
      analysis.absolutePremiumGainPercentage = analysis.allInvestmentsAllTime > 0 
      ...
      if (true) {
        ...
        if (analysis.symbol === "WOLF") {
          console.log(`WOLF current max potential profit calculation:`, { effectiveStrikePrice, strikeValue, sumOfEquityTransactions, netPremiumAllTime: analysis.netPremiumAllTime });
        }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold">Options Analysis - All Symbols ({optionAnalysis.length} symbols)</h2>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-max divide-y divide-gray-200" style={{ minWidth: '2000px' }}>
              <thead className="bg-gray-50">
                <tr>
                  <th 
                    className="sticky left-0 z-10 bg-gray-50 px-1 py- text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200 cursor-pointer"
                    onClick={() => handleSort('symbol')}
                  >
                    Symbol{getSortIndicator('symbol')}
                  </th>
                  <th 
                    className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-300"
                  ...
                  >
                    Net Premium {getMonthNames().currentMonth}{getSortIndicator('netPremiumCurrentMonth')}
                  </th>
                  <th 
                    className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                    onClick={() => handleSort('netPremiumLastMonth')}
                  >
                    Net Premium {getMonthNames().lastMonth}{getSortIndicator('netPremiumLastMonth')}
                  </th>
                  <th 
                    className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                    onClick={() => handleSort('netPremiumTwoMonthsAgo')}
                  >
                    Net Premium {getMonthNames().twoMonthsAgo}{getSortIndicator('netPremiumTwoMonthsAgo')}
                  </th>
                  <th 
                    className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {getSortedData(optionAnalysis)
                  ...
                  <tr key={analysis.symbol} className="hover:bg-gray-200">
                    ...
                    <td className={`${baseRowStyles()} font-medium text-right ${
                    ...
                    <td className={`${baseRowStyles()} text-right ${
                      analysis.netPremiumCurrentMonth >= 0 ? 'text-green-600' : 'text-red-600'
                    }`}>
                      ${formatNumber(analysis.netPremiumCurrentMonth)}
                    </td>
                    <td className={`${baseRowStyles()} text-right ${
                      analysis.netPremiumLastMonth >= 0 ? 'text-green-600' : 'text-red-600'
                    }`}>
                      ${formatNumber(analysis.netPremiumLastMonth)}
                    </td>
                    <td className={`${baseRowStyles()} text-right ${
                      analysis.netPremiumTwoMonthsAgo >= 0 ? 'text-green-600' : 'text-red-600'
                    }`}>
                    ...
                    <td className={`${baseRowStyles()} font-medium text-right ${
                      analysis.absolutePremiumGainPercentage >= 0 ? 'text-green-600' : 'text-red-600'
                    }`}>
                    ...
                    <td className={`${baseRowStyles()} text-right ${
                      analysis.premiumGainedThisWeek >= 0 ? 'text-green-600' : 'text-red-600'
                    }`}>
                      <div className="flex items-center justify-end space-x-1">
                        <span className="text-lg">
                          {
                            analysis.totalLots > 0 && analysis.premiumGainedThisWeek > 0
                              ? `☑️`
                              : analysis.totalLots < 1
                                ? null
                                : '☐'  
                          }
                          {/* {analysis.totalLots < 1 ? 'N/A' : ''}
                          {analysis.totalLots > 0 && analysis.premiumGainedThisWeek > 0 ? '☑️' : '☐'} */}
                        </span>
                        <span>{analysis.totalLots > 0 ? formatNumber(analysis.premiumGainedThisWeek) : 'N/A'}</span>
                      </div>
                    </td>
                    <td className={`${baseRowStyles()} text-right`}>
                      {analysis.totalLots < 1 ? 'N/A' : (
                    ...
                    <td className={`${baseRowStyles()} font-medium text-right ${
                      analysis.currentMaxPotentialProfit >= 0 ? 'text-green-600' : 'text-red-600'
                    }`}>
                      ${formatNumber(analysis.currentMaxPotentialProfit)}
                    </td>
                    <td className={`${baseRowStyles()} font-medium text-right ${
                      analysis.currentMaxGainLossPercentage >= 0 ? 'text-green-600' : 'text-red-600'
                    }`}>
              ...
              <tfoot className="bg-gray-100">
                <tr className="font-semibold">
                  ...
                  <td className={`px-4 py-4 whitespace-nowrap text-sm font-bold text-right ${
                    optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.netPremiumAllTime, 0) >= 0 ? 'text-green-600' : 'text-red-600'
                  }`}>
                    ${formatNumber(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.netPremiumAllTime, 0))}
                  </td>
                  <td className={`px-4 py-4 whitespace-nowrap text-sm font-bold text-right ${
                    optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.netPremiumCurrentMonth, 0) >= 0 ? 'text-green-600' : 'text-red-600'
                  }`}>
                    ${formatNumber(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.netPremiumCurrentMonth, 0))}
                  </td>
                  <td className={`px-4 py-4 whitespace-nowrap text-sm font-bold text-right ${
                    optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.netPremiumLastMonth, 0) >= 0 ? 'text-green-600' : 'text-red-600'
                  }`}>
                    ${formatNumber(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.netPremiumLastMonth, 0))}
                  </td>
                  <td className={`px-4 py-4 whitespace-nowrap text-sm font-bold text-right ${
                    optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.netPremiumTwoMonthsAgo, 0) >= 0 ? 'text-green-600' : 'text-red-600'
                  }`}>
                    ${formatNumber(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.netPremiumTwoMonthsAgo, 0))}
                  </td>
                  <td className="px-4 py-4 whitespace-nowrap text-sm font-bold text-gray-900 text-right">
                    ${formatNumber(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.allInvestmentsAllTime, 0))}
                  </td>
                  <td className="px-4 py-4 whitespace-nowrap text-sm font-bold text-gray-900 text-right">
                    ${formatNumber(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.allSalesAllTime, 0))}
                  </td>
                  <td className={`px-4 py-4 whitespace-nowrap text-sm font-bold text-right ${
                    optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.currentExposure, 0) >= 0 ? 'text-green-600' : 'text-red-600'
                  }`}>
                    ${formatNumber(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.currentExposure, 0))}
                  </td>
                  <td className="px-4 py-4 whitespace-nowrap text-sm font-bold text-gray-900 text-right">
                    ${formatNumber(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.investmentBasisSharePrice, 0) / Math.max(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).length, 1))}
                  </td>
                  <td className="px-4 py-4 whitespace-nowrap text-sm font-bold text-gray-900 text-right">
                    ${formatNumber(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).filter(a => a.currentBasisSharePrice > 0).reduce((sum, a) => sum + a.currentBasisSharePrice, 0) / Math.max(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).filter(a => a.currentBasisSharePrice > 0).length, 1))}
                  </td>
                  <td className={`px-4 py-4 whitespace-nowrap text-sm font-bold text-right ${
                    (optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.absolutePremiumGainPercentage, 0) / Math.max(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).length, 1)) >= 0 ? 'text-green-600' : 'text-red-600'
                  }`}>
                    {formatNumber(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.absolutePremiumGainPercentage, 0) / Math.max(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).length, 1))}%
                  </td>
                  <td className="px-4 py-4 whitespace-nowrap text-sm font-bold text-gray-900 text-right">
                    {formatNumber(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.totalLots, 0), 0)}
                  </td>
                  <td className={`px-4 py-4 whitespace-nowrap text-sm font-bold text-right ${
                    optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.premiumGainedThisWeek, 0) >= 0 ? 'text-green-600' : 'text-red-600'
                  }`}>
                    <div className="flex items-center justify-end space-x-1">
                      <span className="text-lg">
                        {optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.premiumGainedThisWeek, 0) > 0 ? '☑️' : '☐'}
                      </span>
                      <span>${formatNumber(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.premiumGainedThisWeek, 0))}</span>
                    </div>
                  </td>
                  <td className="px-4 py-4 whitespace-nowrap text-sm font-bold text-gray-900 text-right">
                    ${formatNumber(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).filter(a => a.averageStrikePrice > 0).reduce((sum, a) => sum + a.averageStrikePrice, 0) / Math.max(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).filter(a => a.averageStrikePrice > 0).length, 1))}
                  </td>
                  <td className={`px-4 py-4 whitespace-nowrap text-sm font-bold text-right ${
                    optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.currentMaxPotentialProfit, 0) >= 0 ? 'text-green-600' : 'text-red-600'
                  }`}>
                    ${formatNumber(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.currentMaxPotentialProfit, 0))}
                  </td>
                  <td className={`px-4 py-4 whitespace-nowrap text-sm font-bold text-right ${
                    (optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.currentMaxGainLossPercentage, 0) / Math.max(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).length, 1)) >= 0 ? 'text-green-600' : 'text-red-600'
                  }`}>
                    {formatNumber(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).reduce((sum, a) => sum + a.currentMaxGainLossPercentage, 0) / Math.max(optionAnalysis.filter(analysis => !hideZeroLots || analysis.totalLots > 0).length, 1))}%
                  </td>
                </tr>
              </tfoot>
            </table>
